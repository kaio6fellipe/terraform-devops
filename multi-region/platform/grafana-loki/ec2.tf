module "ec2-instance01" {
  ...
  vpc_security_group_ids = ["${aws_security_group.loki_cluster_instance.id}", "${aws_security_group.loki_http_instance.id}", "${var.allow_bastion_ssh}", "${var.allow_ansible_admin_subnet_ssh}", "${var.allow_outbound}"]
