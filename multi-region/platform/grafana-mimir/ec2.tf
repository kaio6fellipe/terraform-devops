module "ec2-instance01" {
  ...
  vpc_security_group_ids = ["${aws_security_group.mimir_cluster_instance.id}", "${aws_security_group.mimir_http_instance.id}", "${var.allow_bastion_ssh}", "${var.allow_ansible_admin_subnet_ssh}", "${var.allow_outbound}"]
