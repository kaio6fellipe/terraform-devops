resource "aws_s3_bucket" "mimir-blocks" {
  ...
  bucket        = local.bucket_names.blocks
  force_destroy = true
}

resource "aws_s3_bucket_public_access_block" "block-blocks" {
  bucket = aws_s3_bucket.mimir-blocks.id
...
resource "aws_s3_bucket_policy" "grant-access-blocks" {
  bucket = aws_s3_bucket.mimir-blocks.id
  policy = jsonencode({
    Version : "2012-10-17",
    Statement : [
