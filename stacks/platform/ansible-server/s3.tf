resource "aws_s3_bucket" "codepipeline_bucket" {
  ...
  bucket        = "codepipeline-ansible-ktech-br-${local.globals.environment}"
  force_destroy = true
  ...
  logging {
    target_bucket = "codepipeline-ansible-ktech-br-${local.globals.environment}"
    target_prefix = "bucket-access-log/codepipeline-ansible-ktech-br-${local.globals.environment}"
  }
}
