resource "aws_s3_bucket" "mimir-alertmanager" {
  ...
  bucket        = local.bucket_names.alertmanager
  force_destroy = true
}

resource "aws_s3_bucket_public_access_block" "block-alertmanager" {
  bucket = aws_s3_bucket.mimir-alertmanager.id
...
resource "aws_s3_bucket_policy" "grant-access-alertmanager" {
  bucket = aws_s3_bucket.mimir-alertmanager.id
  policy = jsonencode({
    Version : "2012-10-17",
    Statement : [
